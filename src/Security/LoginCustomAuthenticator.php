...
class LoginCustomAuthenticator extends AbstractFormLoginAuthenticator implements PasswordAuthenticatedInterface
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, $providerKey)
    {
      ...
      return new RedirectResponse($this->urlGenerator->generate('app_admin'));
     ...
     else if($users->getRoles()==['ROLE_ETUDIANT'] && $users->getActivationToken()==null && $users->getCount()!= 0){
        return new RedirectResponse($this->urlGenerator->generate('app_etudiant'));
          ...
          return new RedirectResponse($this->urlGenerator->generate('app_upload_etudiant'));

      }


     else if($users->getRoles()==['ROLE_ENTREPRISE'] && $users->getActivationToken()==null){
